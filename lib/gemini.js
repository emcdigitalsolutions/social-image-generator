...
async function callGemini(apiKey, systemInstruction, userPrompt, options = {}) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (json.error) {
      ...
      // Errore HARD di billing/credito esaurito: NON è transiente, inutile (e dannoso)
      // ritentare 5×30s — la richiesta resterebbe appesa ~2,5 min e il browser/proxy
      // chiuderebbe la connessione mostrando "Failed to fetch". Falliamo subito con un
      // messaggio chiaro e azionabile.
      const isBilling = lower.includes('prepayment') || lower.includes('credits are depleted')
        || lower.includes('credit') && lower.includes('deplet') || lower.includes('billing');
      if (isBilling) {
        throw new Error('Crediti AI Gemini esauriti: il piano a pagamento Google è terminato. Ricarica il credito su Google AI Studio (ai.studio) oppure inserisci una nuova chiave Gemini valida in Impostazioni → AI. La generazione automatica è temporaneamente non disponibile (puoi comunque scrivere la caption a mano).');
      }
...
REGOLE DI LUNGHEZZA (IMPORTANTISSIMO):
- Il post deve essere BREVE e scorrevole: massimo 3-4 frasi nel corpo, circa 40-70 parole. Le persone si stancano a leggere post lunghi sui social.
