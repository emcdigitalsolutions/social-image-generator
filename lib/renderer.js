...
// DB fallback: load client defaults from database if not in hardcoded list
function getClientDefaults(clientId) {
  if (CLIENT_DEFAULTS[clientId]) return CLIENT_DEFAULTS[clientId];
  try {
    const { getDb } = require('./db');
    const db = getDb();
    const client = db.prepare('SELECT brand_name, website, tagline, logo_filename FROM clients WHERE id = ?').get(clientId);
    if (client) {
      return {
        brand_name: client.brand_name || client.display_name || clientId,
        website: client.website || '',
        tagline: client.tagline || '',
        logo: client.logo_filename || '',
      };
    }
  } catch { /* DB not available, use empty defaults */ }
  return {};
}
...
async function loadTemplate(templateName, client) {
  ...
  const themePath = path.join(TEMPLATES_DIR, 'themes', `${client}.css`);
  ...
  // Load client theme CSS (overrides base variables)
  let themeCss = '';
  try {
    themeCss = await fs.readFile(themePath, 'utf-8');
  } catch { /* no theme file — use base defaults */ }
