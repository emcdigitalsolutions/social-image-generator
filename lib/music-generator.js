...
// Cache della latest version del modello per non fare un GET extra a ogni
// generazione. Replicate aggiorna le versioni raramente.
let _cachedModelVersion = null;
let _cachedModelVersionAt = 0;
const VERSION_CACHE_MS = 60 * 60 * 1000; // 1 ora

async function _getMusicgenVersion(token) {
  const now = Date.now();
  if (_cachedModelVersion && (now - _cachedModelVersionAt) < VERSION_CACHE_MS) {
    return _cachedModelVersion;
  }
  const res = await _httpsRequest({
    host: REPLICATE_HOST,
    path: '/v1/models/meta/musicgen',
    headers: { 'Authorization': `Token ${token}` }
  });
  const text = res.body.toString('utf-8');
  if (res.status !== 200) {
    throw new Error(`Replicate get-model ${res.status}: ${text.substring(0, 200)}`);
  }
  const json = JSON.parse(text);
  const version = json.latest_version && json.latest_version.id;
  if (!version) throw new Error('Replicate: latest_version mancante per meta/musicgen');
  _cachedModelVersion = version;
  _cachedModelVersionAt = now;
  return version;
}

async function _replicateSubmit(token, prompt, durationSec) {
  const version = await _getMusicgenVersion(token);
  // Endpoint generale predictions: richiede version explicit nel body.
