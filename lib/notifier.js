...
async function sendInsightsReport({ client, recipient, periodLabel, summary, pdfBuffer, filename }) {
  ...
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;font-size:14px;color:#111;line-height:1.5">
      ...
      <p style="margin:18px 0 0">Grazie per la collaborazione,</p>
      ${emcEmailSignature()}
    </div>`;
 ...
 */
/**
 * Invia al cliente il link del questionario di onboarding (usa contact_email).
 * EMC sempre in CC. GAS relay preferito, fallback SMTP.
 */
async function sendQuestionnaireLinkToClient({ client, questionnaireUrl }) {
  const recipient = client && client.contact_email;
  if (!recipient) {
    return { skipped: true, reason: 'no-contact-email' };
  }
  const clientName = client.display_name || client.id;
  const subject = `Qualche domanda per conoscerti meglio — ${clientName}`;
  const html = `
    <div style="font-family:Inter,Arial,sans-serif;font-size:14px;color:#111;line-height:1.55;max-width:580px;margin:0 auto">
      <div style="background:#0f1e4a;color:white;padding:18px 22px;border-radius:8px 8px 0 0">
        <h1 style="margin:0;font-size:18px;font-weight:700">Il questionario per la tua comunicazione</h1>
        <div style="opacity:0.85;font-size:12px;margin-top:4px">${clientName}</div>
      </div>
      <div style="border:1px solid #e5e7eb;border-top:0;padding:22px;border-radius:0 0 8px 8px;background:white">
        <p>Ciao!</p>
        <p>Per raccontare al meglio la tua attività sui social ci servono alcune informazioni:
        abbiamo preparato un breve questionario (5-10 minuti). Le tue risposte ci aiutano a
        creare contenuti su misura, col tuo tono e i tuoi obiettivi.</p>
        <div style="text-align:center;margin:28px 0">
          <a href="${questionnaireUrl}" style="display:inline-block;background:#2563eb;color:white;text-decoration:none;padding:13px 28px;border-radius:6px;font-weight:600;font-size:15px">
            Compila il questionario →
          </a>
        </div>
        <p style="font-size:12px;color:#6b7280">Puoi compilarlo dal telefono o dal computer, quando preferisci.
        Se il bottone non funziona, copia questo link nel browser:<br>
        <a href="${questionnaireUrl}" style="color:#2563eb;word-break:break-all">${questionnaireUrl}</a></p>
        <p style="margin-top:20px">Grazie!<br><strong>EMC Digital Solutions</strong><br>
        <span style="font-size:12px;color:#6b7280">www.emcdigitalsolutions.it · emcdigitalsolution@gmail.com</span></p>
      </div>
    </div>`;
  const cc = ownerCc(recipient); // EMC sempre in copia (regola CC)
  ...
  if (getGasEmailUrl()) {
    try {
      ...
      console.log(`[notifier] Link questionario inviato via GAS a ${recipient}${cc ? ' (cc: ' + cc + ')' : ''}`);
      return { sent_to: recipient };
    } catch (err) {
      console.warn(`[notifier] GAS relay fallito per questionario: ${err.message} — tento SMTP`);
    }
  }
  const t = getTransporter();
  if (!t) throw new Error('Email non inviata: GAS email relay non configurato e SMTP non disponibile');
  ...
  if (cc) mail.cc = cc;
  await t.sendMail(mail);
  console.log(`[notifier] Link questionario inviato via SMTP a ${recipient}${cc ? ' (cc: ' + cc + ')' : ''}`);
  return { sent_to: recipient };
}
  ...
  return `
    <div style="font-family:Inter,Arial,sans-serif;font-size:14px;color:#111;line-height:1.55;max-width:580px;margin:0 auto">
      ...
      <div style="border:1px solid #e5e7eb;border-top:0;padding:22px;border-radius:0 0 8px 8px;background:white">
        ...
        <p style="margin:22px 0 0">Grazie!</p>
        ${emcEmailSignature()}
      </div>
    </div>`;
}
...
// Recap del questionario compilato dal cliente. Inviato a EMC (destinatario) con
// il cliente in CC (se ha contact_email), così entrambi ne hanno traccia.
// responses = oggetto { "Domanda": "risposta" | ["a","b"], ... } (chiavi già leggibili).
async function sendQuestionnaireRecap({ client, questionnaire, responses }) {
  const clientName = (client && (client.display_name || client.brand_name)) || 'Cliente';
  const ccClient = client && client.contact_email ? client.contact_email.trim() : null;
  const subject = `Questionario social compilato — ${clientName}`;

  const entries = responses && typeof responses === 'object' ? Object.entries(responses) : [];
  let rows;
  if (!entries.length) {
    rows = '<tr><td style="padding:12px 14px;color:#6b7280">Nessuna risposta registrata.</td></tr>';
  } else {
    rows = entries.map(([q, a]) => {
      const ans = Array.isArray(a) ? a.join(', ') : String(a == null ? '' : a);
      return `<tr>
        <td style="padding:10px 14px;border-bottom:1px solid #eef0f4;font-weight:600;color:#1f2937;vertical-align:top;width:42%">${escapeHtmlEmail(q)}</td>
        <td style="padding:10px 14px;border-bottom:1px solid #eef0f4;color:#374151;white-space:pre-wrap">${escapeHtmlEmail(ans)}</td>
      </tr>`;
    }).join('');
  }

  const submitted = (questionnaire && questionnaire.submitted_at) || '';
  const sector = questionnaire && questionnaire.sector ? ' · Settore: ' + escapeHtmlEmail(questionnaire.sector) : '';
  const html = `
    <div style="font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;max-width:640px;margin:0 auto">
      <h2 style="color:#2563eb;margin:0 0 4px">Questionario compilato</h2>
      <p style="color:#374151;font-size:14px;margin:0 0 16px">Cliente: <strong>${escapeHtmlEmail(clientName)}</strong>${sector}${submitted ? ' · Inviato il ' + escapeHtmlEmail(submitted) : ''}</p>
      <table style="border-collapse:collapse;width:100%;font-size:14px;border:1px solid #eef0f4;border-radius:10px;overflow:hidden">${rows}</table>
      ${emcEmailSignature()}
    </div>`;

  // sendNotification invia all'owner EMC; il cliente va in CC (se presente).
  return sendNotification(subject, html, ccClient);
}

module.exports = { sendNotification, notifyPublishFailed, notifyPublishPartial, notifyApprovalAction, notifyApprovalSummary, sendInsightsReport, sendApprovalLinkToClient, sendQuestionnaireLinkToClient, sendSinglePostNotification, sendQuestionnaireRecap };
