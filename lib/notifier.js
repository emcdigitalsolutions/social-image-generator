 ...
 */
function sendViaGasEmailRelay({ to, subject, html, attachment, replyTo }) {
  return new Promise((resolve, reject) => {
...
async function sendInsightsReport({ client, recipient, periodLabel, summary, pdfBuffer, filename }) {
  ...
  if (getGasEmailUrl()) {
    try {
      await sendViaGasEmailRelay({ to: recipient, subject, html, attachment });
      console.log(`[notifier] Report PDF inviato via GAS a ${recipient} (${pdfBuffer.length} bytes)`);
      return;
    } catch (err) {
  ...
  console.log(`[notifier] Report PDF inviato via SMTP a ${recipient} (${pdfBuffer.length} bytes)`);
}
...
async function sendApprovalLinkToClient({ client, approvalUrl, planTitle, monthNumber, expiresAt, totalPosts }) {
  ...
  if (getGasEmailUrl()) {
    try {
      await sendViaGasEmailRelay({ to: recipient, subject, html });
      console.log(`[notifier] Approval link inviato via GAS a ${recipient} per ${client.display_name || client.id} mese ${monthNumber}`);
  ...
  console.log(`[notifier] Approval link inviato via SMTP a ${recipient}`);
  return { sent_to: recipient };
}
