...
function authMiddleware(req, res, next) {
  // Accetta sia header Bearer (chiamate API client-side via apiFetch) che
  // cookie dashboard_token (apertura diretta in browser, es. download PDF
  // via window.open). Stesso JWT, due trasporti diversi.
  let token = null;
  const header = req.headers.authorization;
  if (header && header.startsWith('Bearer ')) {
    token = header.slice(7);
  } else if (req.cookies && req.cookies.dashboard_token) {
...
function pageAuthMiddleware(req, res, next) {
  ...
  // Refresh del cookie ad ogni page load: fissa eventuali flag legacy
  // (es. SameSite=Strict che blocca i target=_blank su mobile) con la
  // policy attuale Lax. Idempotente: stesso valore, flag aggiornato.
  try {
  ...
  } catch (_) { /* cookie-parser potrebbe non esporre res.cookie in alcuni casi */ }
  req.user = payload;
  next();
}
