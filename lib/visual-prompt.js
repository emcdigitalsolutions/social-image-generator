...
// Genera 5 STILI VISIVI riutilizzabili a partire dal brand del cliente.
// Output: array di 5 stringhe in inglese, ognuna una direttiva di STILE
// (lighting + composition + mood + quality), NON una scena specifica — così
// possono ruotare su qualsiasi post. Pensato per popolare la textarea in UI,
// l'utente poi conferma con Salva.
const STYLES_SYSTEM_INSTRUCTION = `Sei un art director esperto di fotografia editoriale per social media.
Dato il profilo di un brand, generi 5 STILI VISIVI fotografici riutilizzabili.

Output: ESATTAMENTE 5 righe, una per stile, in INGLESE. Nessuna numerazione, nessun prefisso, nessuna riga vuota.

Ogni stile è una DIRETTIVA DI STILE (non una scena): deve descrivere lighting, composition, mood, e quality keywords.
NON includere il soggetto specifico (es. niente "honey jar" o "coffin"): lo stile deve poter ruotare su qualsiasi contenuto del brand.
I 5 stili devono essere chiaramente DIVERSI tra loro (es. macro, flat-lay, lifestyle candid, cinematic wide, studio) per garantire varietà visibile.
Coerenza brand: per attività artigianali/locali usa toni autentici e caldi, non plastica/stock. Se forniti brand_colors, puoi accennare alla palette.
Ogni riga max 280 caratteri.

Esempio output (per un'apicoltura):
warm artisanal macro photography, golden hour side light, extreme detail, shallow depth of field, soft amber bokeh, photorealistic, authentic rustic mood
minimal flat lay top-down, natural wood and linen surfaces, soft diffused daylight, earthy palette, magazine editorial composition, photorealistic
lifestyle candid documentary, soft window light, genuine spontaneous moments, slight film grain, warm inviting atmosphere, cinematic
cinematic wide establishing shot, countryside golden light, atmospheric depth, moody natural color grading, anamorphic feel, photorealistic
clean studio product photography, neutral warm background, three-point softbox lighting, sharp focus, refined commercial polish, photorealistic`;

async function generateVisualStyles(client) {
  const apiKey = getEffectiveGeminiKey(client);
  if (!apiKey) {
    const err = new Error('Nessuna Gemini API key configurata (cliente o globale)');
    err.code = 'NO_API_KEY';
    throw err;
  }
  ...
  const parts = [];
  if (client.brand_name)   parts.push(`brand="${client.brand_name}"`);
  else if (client.display_name) parts.push(`brand="${client.display_name}"`);
  if (client.sector)       parts.push(`settore="${client.sector}"`);
  if (client.location)     parts.push(`località="${client.location}"`);
  if (client.tagline)      parts.push(`tagline="${client.tagline}"`);
  const brandColors = parseBrandColors(client.brand_colors);
  if (brandColors.length)  parts.push(`brand_colors=${brandColors.join(',')}`);
  const userPrompt = parts.length ? parts.join(', ') : 'brand generico, settore lifestyle';

  const { text } = await callGemini(apiKey, STYLES_SYSTEM_INSTRUCTION, userPrompt, {
    temperature: 0.9,
    maxTokens: 700
  });
  ...
  const lines = (text || '')
    ...
    .map(l => l.replace(/^[0-9]+[.)\-\s]+/, '').replace(/^["'`]|["'`]$/g, '').trim())
    .filter(l => l.length > 5)
    .map(l => l.substring(0, 500))
    .slice(0, 5);

  if (!lines.length) {
    const err = new Error('Generazione stili non riuscita: risposta AI vuota');
