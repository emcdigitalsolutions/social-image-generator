...
async function publishPost(client, post, media, opts = {}) {
  ...
  const results = { fb_post_id: null, ig_media_id: null, linkedin_post_id: null, tiktok_publish_id: null, errors: [], channels };
  ...
  if (wantFb || wantIg) {
    ...
    if (missing.length) {
      ...
      // Se SOLO Meta è chiesto, abort. Se anche LinkedIn/TikTok sono chiesti, continua per quelli.
      if (!wantLinkedIn && !wantTikTok) return results;
    } else {
