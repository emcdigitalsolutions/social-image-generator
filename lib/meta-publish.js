...
async function publishToInstagram(client, post, media) {
  ...
  if (post.media_type === 'video' || post.media_type === 'reel') {
    const m = media[0];
    // 'video' → share anche nel feed (default true).
    // 'reel'  → potresti volerlo solo in tab Reels; per ora share_to_feed=true
    //          per uniformità, in futuro distinguibile via opzione utente.
