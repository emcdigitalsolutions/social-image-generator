...
async function publishPost(client, post, media) {
  ...
  // Scambio token system user → page token UNA SOLA VOLTA, serve sia FB che IG
  ...
  // IG: basta avere Page Token (scope IG) e ig_user_id.
  // Richiede che l'IG Business sia collegato alla Page FB del cliente.
  if (client._fb_page_token && client.ig_user_id) {
    try {
