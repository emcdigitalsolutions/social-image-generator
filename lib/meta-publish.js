...
async function publishPost(client, post, media, opts = {}) {
  ...
  // Pre-check credenziali: se l'utente ha chiesto un canale ma il cliente non
  // ha i campi Meta necessari, logga errore esplicito invece di uscire silent.
  // Senza questo check, publishPost può ritornare {fb_post_id:null, ig_media_id:null, errors:[]}
  // facendo credere allo scheduler che la pubblicazione sia andata a buon fine.
  const missing = [];
  if (wantFb && !client.fb_page_id) missing.push('fb_page_id');
  if ((wantFb || wantIg) && !client.fb_system_user_token) missing.push('fb_system_user_token');
  ...
  // Safety net: se nessuno dei due branch ha girato (fb_post_id e ig_media_id
  // sono null E non abbiamo errori accumulati), registra un errore sentinella.
  // Non dovrebbe mai succedere con il pre-check sopra, ma copre il caso in cui
  // `channels` sia vuoto o condizioni impreviste saltino entrambi i branch.
  if (!results.fb_post_id && !results.ig_media_id && !results.errors.length) {
