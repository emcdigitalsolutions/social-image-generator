...
const MAX_BACKUPS = 7;        // copie DB (giornaliere, ~17MB l'una)
const MAX_MEDIA_BACKUPS = 3;  // copie media (settimanali, ~1GB l'una)
...
function rotate(prefix, ext, max = MAX_BACKUPS) {
...
function backupMedia(stamp) {
  ...
  rotate('media_', '.tar.gz', MAX_MEDIA_BACKUPS);
}

/**
 * Esegue il backup.
 * - DB: sempre (file piccolo, è il dato prezioso).
 * - Media: solo se `includeMedia` è true — il tar.gz pesa ~1GB, quindi:
 *   al BOOT si salta (ogni deploy ne creerebbe uno), e il cron giornaliero
 *   lo fa solo la DOMENICA (retention MAX_MEDIA_BACKUPS copie ≈ 3 settimane).
 */
function runBackup({ includeMedia = false } = {}) {
  try {
    if (!fs.existsSync(BACKUPS_DIR)) fs.mkdirSync(BACKUPS_DIR, { recursive: true });
    const stamp = timestamp();
    backupDb(stamp);
    if (includeMedia) backupMedia(stamp);
  } catch (err) {
