...
/**
 * Se il file è un PNG con canale alpha (trasparenza), lo flatten contro
 * sfondo bianco e sovrascrive. IG rifiuta i PNG con alpha sui post feed
 * single image (errore 9004 / "Only photo or video can be accepted").
 *
 * Idempotente: se non è PNG o non ha alpha, no-op.
 */
async function flattenPngAlpha(filePath) {
  try {
    const sharp = require('sharp');
    const meta = await sharp(filePath).metadata();
    if (meta.format !== 'png' || !meta.hasAlpha) return { changed: false, reason: 'no-alpha-or-not-png' };
    const buf = await sharp(filePath)
      .flatten({ background: '#ffffff' })
      .png({ compressionLevel: 9, palette: false })
      .toBuffer();
    fs.writeFileSync(filePath, buf);
    return { changed: true, format: meta.format, hadAlpha: true };
  } catch (err) {
    console.warn('[post-media] flattenPngAlpha failed:', filePath, err.message);
...
async function attachUploadedFile({ clientId, postId, tmpPath, originalName, mimetype, source = 'upload' }) {
  ...
  // PNG con alpha → flatten contro bianco (IG rifiuta PNG trasparenti)
  if (cls.kind === 'image') {
    await flattenPngAlpha(dest);
  }
...
module.exports = {
  ...
  validateForMediaType, flattenPngAlpha,
