...
const MAX_VIDEO_BYTES = 250 * 1024 * 1024;   // 250 MB (reel montati da CapCut/Canva in alta qualità)
...
async function attachUploadedFile({ clientId, postId, tmpPath, originalName, mimetype, source = 'upload' }) {
  ...
  // Video: analizza con ffprobe e ri-encoda IG-safe se non conforme (HEVC,
  // 4K, 10-bit, audio non-AAC, dimensioni dispari...). Best-effort: se ffmpeg
  // manca o il re-encode fallisce, teniamo l'originale + warning diagnostico.
  if (cls.kind === 'video') {
    try {
      const { normalizeVideoForMeta } = require('./video-normalize');
      const result = await normalizeVideoForMeta(dest);
      if (result.width) width = result.width;
      if (result.height) height = result.height;
      if (result.changed) {
        console.log(`[post-media] video ${filename} ri-encodato IG-safe — issue: ${result.issues.join('; ')}`);
      }
      if (result.error) {
        warnings.push(`Video non normalizzato (${result.error}). Verrà pubblicato così com'è: se Meta lo rifiuta, ri-encodalo manualmente.`);
      } else if (!result.changed && result.warnings && result.warnings.length) {
        warnings.push(...result.warnings);
      }
    } catch (err) {
      console.warn('[post-media] normalizeVideoForMeta failed for', filename, ':', err.message);
      warnings.push('Analisi video non riuscita: ' + err.message);
    }
  }
  ...
  // Bytes possono essere cambiati dopo flatten (immagini) o re-encode (video) — rileggiamo
