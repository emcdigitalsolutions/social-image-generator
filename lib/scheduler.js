...
// Lock PERSISTENTE su DB (claim atomico): previene il doppio publish dello stesso
// post sia tra tick sovrapposti dello stesso processo, sia — soprattutto — tra
// PIÙ CONTAINER che condividono lo stesso DB (es. deploy rolling che lascia 2
// container attivi). Il claim è un UPDATE atomico `status='ready' -> 'publishing'`:
// SQLite serializza le scritture, quindi un solo worker/container "vince" il post.
// Recovery: un post rimasto 'publishing' oltre PUBLISHING_STALE_MIN (worker morto
// durante il publish) viene riportato a 'ready' e ritentato.
const PUBLISHING_STALE_MIN = 15;
...
async function checkAndPublish() {
  ...
  // Recovery: post rimasti 'publishing' (claim non rilasciato perché il worker è
  // morto a metà publish) oltre la soglia → riportali a 'ready' per ritentare.
  try {
    const recovered = db.prepare(
      `UPDATE posts SET status='ready', updated_at=datetime('now')
       WHERE status='publishing' AND updated_at < datetime('now', ?)`
    ).run(`-${PUBLISHING_STALE_MIN} minutes`);
    if (recovered.changes) console.warn(`[scheduler] Recovered ${recovered.changes} post bloccati in 'publishing'`);
  } catch (e) { console.error('[scheduler] recovery publishing failed:', e.message); }
  ...
  for (const post of posts) {
    // Claim atomico: prova a "prendere" il post passandolo da 'ready' a 'publishing'.
    // Se changes===0, un altro tick o un ALTRO CONTAINER l'ha già preso → skip.
    // Questo è l'anti-doppione che funziona anche tra container (lock su DB).
    const claim = db.prepare(
      "UPDATE posts SET status='publishing', updated_at=datetime('now') WHERE id=? AND status='ready'"
    ).run(post.id);
    if (claim.changes === 0) {
      console.log(`[scheduler] Skip post ${post.id}: già preso da un altro worker/container`);
      continue;
    }
...
module.exports = { start, stop, checkAndPublish, refreshAllInsights, sendMonthlyReports };
