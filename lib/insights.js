...
// Metriche giornaliere per Page FB. Una chiamata batch con period=day.
const FB_ACCOUNT_METRICS = ['page_impressions', 'page_impressions_unique', 'page_views_total', 'page_post_engagements'];

// Metriche giornaliere per IG Business. v22+: 'impressions' sostituito da 'views'.
const IG_ACCOUNT_METRICS = ['reach', 'profile_views'];
...
async function fetchFbAccountInsights(pageToken, pageId) {
  ...
  // Insights batch: period=day; la response torna data[] per ogni metrica
  try {
    const metrics = FB_ACCOUNT_METRICS.join(',');
    const r = await httpGet(FB_HOST, `/${GRAPH_API_VERSION}/${pageId}/insights?metric=${metrics}&period=day&access_token=${encodeURIComponent(pageToken)}`);
    if (r.error) {
      // non sovrascriviamo un errore già presente (es. fan_count)
      if (!error) error = r.error.message;
    } else {
      raw.insights = r.data;
      out.impressions   = extractValue(r.data, 'page_impressions');
      out.reach         = extractValue(r.data, 'page_impressions_unique');
      out.profile_views = extractValue(r.data, 'page_views_total');
      out.engaged_users = extractValue(r.data, 'page_post_engagements');
    }
  } catch (err) { if (!error) error = err.message; }
...
async function fetchIgAccountInsights(pageToken, igUserId) {
  ...
  // Daily insights. Una chiamata per metrica per evitare che 1 metrica invalida
  // faccia fallire tutto (stessa strategia di fetchMetricsIndividually).
  raw.insights = {};
  for (const metric of IG_ACCOUNT_METRICS) {
    try {
      const r = await httpGet(FB_HOST, `/${GRAPH_API_VERSION}/${igUserId}/insights?metric=${metric}&period=day&access_token=${encodeURIComponent(pageToken)}`);
      if (r.error) {
      ...
      const v = extractValue(r.data, metric);
      if (metric === 'reach') out.reach = v;
      if (metric === 'profile_views') out.profile_views = v;
    } catch (err) {
...
module.exports = {
  ...
  FB_ACCOUNT_METRICS, IG_ACCOUNT_METRICS
};
