...
function clientReplacements(client) {
  ...
  // Normalizza per WhatsApp link: solo cifre, prefisso +39 di default se manca
  let whatsapp_link = '';
  if (phone) {
    let digits = phone.replace(/[^\d]/g, '');
    if (digits && !phone.startsWith('+') && digits.length === 10 && /^[3]/.test(digits)) {
      // numero italiano cellulare senza prefisso → aggiungi 39
      digits = '39' + digits;
    } else if (digits && phone.startsWith('+')) {
      // già con prefisso internazionale
    }
    if (digits) whatsapp_link = 'https://wa.me/' + digits;
  }
  return {
