...
async function createSlideshow(imagePaths, opts) {
  ...
  const args = [];
  // Input ciascuna immagine come loop di clipDuration secondi
  for (const img of imagePaths) {
    args.push('-loop', '1', '-t', String(clipDuration), '-i', img);
  }
  // Audio opzionale: input addizionale, poi -shortest per troncarlo alla durata video
  let audioInputIdx = -1;
  ...
  if (audioInputIdx >= 0) {
    args.push('-map', `${audioInputIdx}:a`, '-shortest', '-c:a', 'aac', '-b:a', '128k');
  } else {
    // Niente audio: aggiungo traccia silenzio per compatibilità Meta upload
    // (alcuni endpoint IG rifiutano video senza audio track).
    args.push('-f', 'lavfi', '-i', 'anullsrc=channel_layout=stereo:sample_rate=44100');
    args.push('-map', `${numClips}:a`, '-shortest', '-c:a', 'aac', '-b:a', '128k');
  }
  ...
  args.push(
    ...
    '-preset', 'medium',
    '-crf', '23',
    '-pix_fmt', 'yuv420p',  // necessario per compatibilità web/Meta
    '-movflags', '+faststart',  // metadata in testa, streaming-friendly
    '-r', String(FPS),
