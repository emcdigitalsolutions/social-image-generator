...
function buildInsightsReportHtml(client, periodLabel, summary, history, topPosts) {
  const today = new Date().toLocaleDateString('it-IT');
...
<style>
  ...
  <div class="intro">Questo documento riassume i risultati delle attività social del periodo indicato: crescita follower, persone raggiunte e contenuti più performanti. I dati sono raccolti automaticamente ogni giorno dalle piattaforme Meta.</div>
...
<script>
  ...
  if (D.labels.length) {
    ...
    new Chart(document.getElementById('ch-followers'), {
      type: 'line',
      data: { datasets: [
        { label: 'Facebook',  data: buildSeries(D.fbFans),       borderColor: '#1877f2', backgroundColor: 'rgba(24,119,242,0.1)', fill: false, tension: 0.3, spanGaps: true, pointRadius: 2 },
        { label: 'Instagram', data: buildSeries(D.igFollowers),  borderColor: '#e4405f', backgroundColor: 'rgba(228,64,95,0.1)',  fill: false, tension: 0.3, spanGaps: true, pointRadius: 2 }
      ]},
      options: fOpt
    });
...
async function renderInsightsReportPdf(client, periodLabel, summary, history, topPosts) {
  const t0 = Date.now();
  console.log('[pdf-insights] start', client.id, periodLabel);
  const html = buildInsightsReportHtml(client, periodLabel, summary, history, topPosts);
  const b = await getBrowser();
  const page = await b.newPage();
  try {
