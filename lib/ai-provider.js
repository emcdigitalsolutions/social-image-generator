...
function generateEditorialPlan(client, questionnaireResponses, months = 6, postsPerMonth, startYearMonth) {
  ...
  if (client.ai_provider === 'claude') {
    return claudePlan(client, questionnaireResponses, months, ppm, startYearMonth);
  }
  return geminiPlan(client, questionnaireResponses, months, ppm, startYearMonth);
}
...
8. **LUNGHEZZA**: Una regola di lunghezza FISSA e non negoziabile per ogni post
...
- Includi SEMPRE, nella sezione LUNGHEZZA, questa regola testuale: "LUNGHEZZA DEI POST (REGOLA FISSA): scrivi sempre post brevi e scorrevoli, massimo 3-4 frasi (circa 40-70 parole). Un solo messaggio chiaro per post, dritto al punto: le persone si stancano a leggere testi lunghi sui social."
